...
export async function draftBrief({ threadId, email, subject, vault }) {
  ...
  if (!data.docx_base64) {
    return { agent: true, refused: !output, message: output || "The brief service returned no brief. No file was written." };
  }
  ...
  return { agent: true, file, message: output || `Brief for ${subject} is ready.` };
}
