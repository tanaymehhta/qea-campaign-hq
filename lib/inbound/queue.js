import { db } from "../db";
import { locate, repsFor } from "./routing";
...
export const CO_LANES = [
  ...
  // Ten companies were typed in by hand to test the research, people and email
  // stages — checked against inbound_webhook_events, which holds no payload for
  // any of them. They never visited, so they are not inbound and they are not
  // in the lanes above. They are also where most of the work is: BXP has 25
  // buildings researched, Durst holds the only five drafts that pass the send
  // gate. Excluding them made the queue look empty; mixing them in would have
  // claimed a visit that never happened. Their own lane says which they are.
  { id: "unvisited", label: "Added by hand — never visited" },
];
...
export async function loadQueue() {
  ...
  const bitesOf = new Map(), reportsOf = new Map(), claimOf = new Map();
  for (const h of hits) {
    if (!h.rule_name) continue;
    const to = ruleById.get(h.rule_id)?.has_teeth ? bitesOf : reportsOf;
    if (!to.has(h.company_id)) to.set(h.company_id, h.rule_name);
  }
  const ruleOf = new Map();
  for (const id of new Set([...bitesOf.keys(), ...reportsOf.keys()])) {
    ruleOf.set(id, bitesOf.get(id) ?? `${reportsOf.get(id)} (reporting only)`);
  }
  ...
  const companyLeads = companies
    .map((c) => {
      // No visit means no RB2B payload — checked against inbound_webhook_events,
      // which holds nothing for any of the ten. They were typed in to test the
      // pipeline. They are still real accounts researched in full, so they get a
      // lane of their own rather than a filter that deletes them.
      const visited = visitsAt.has(c.id);
      ...
      const anon = leads.find((l) => l.identity === "anon" && l.company?.id === c.id);
      const { region, place, basis } = anon ?? locate(null, c, builtAt.get(c.id) ?? []);
      ...
      return {
        ...
        region, place: place || anon?.place || "", basis,
...
export async function loadPerson(personId) {
  ...
  const [company, visits, emails, buildings, hits, signals, coPeople, coVisits, coDrafts] = await Promise.all([
    ...
    // a person's page as it does on the company's.
    cid ? db.from("inbound_people").select("id,email,email_status").eq("company_id", cid).then((r) => r.data ?? []) : [],
