...
export async function loadCompany(companyId) {
  ...
  return {
    ...
    dots: timeline(
      ...
      // Only while it has not come back: a request whose run has already
      // started is being reported by the run itself, and two sources for one
      // fact is how a page starts contradicting itself.
      asked && !runs.some((r) => r.started_at >= asked.requested_at) ? asked : null),
  };
}
