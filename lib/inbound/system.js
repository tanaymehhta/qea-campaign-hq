...
import { loadQueue, timeline, DOTS, DROPOFF, stoppedAt, tally, everyRow } from "./queue.js";
...
export async function loadSystem() {
  ...
  return {
    ...
    // Where each company stopped, counted off the queue's own array rather than
    // a fifth round-trip to recount what is already in memory. `stops` is left
    // behind: the page renders a count and a link, and the predicate lives in
    // `VIEWS` under the same id so the link opens exactly what the bar counted.
    dropoff: DROPOFF.map(({ id, label, fix }) => ({
      id, label, fix,
      n: queue.companies.filter((l) => stoppedAt(l) === id).length,
    })),
