 ...
 */
const REASONS = [
  // Our own cap, not Apollo's refusal — 150 reveals a day, spent elsewhere
  // before this company's turn. It is tested first because the message carries
  // the word "apollo" and would otherwise be read as an empty account, which is
  // a different problem with a different fix: wait for tomorrow, or raise the
  // cap, rather than buy credits.
  [/apollo daily cap/i,
   "our own daily Apollo limit was already spent, so no email addresses were revealed"],
...
/**
 * What the research chip says while research is running.
 *
 * `researchChip` reads `account_type_reason`, which still holds last time's
 * answer — or last time's 402 — for as long as the new run takes. A chip
 * reading "Research failed" beside a spinning dot is the same stale-beside-live
 * reading the timeline and the red boxes were fixed for.
 *
 * Deliberately not a fourth branch inside `researchChip`: that function also
 * feeds the header counts and the queue's filters, and "Research failed: 12"
 * counts a stored state rather than a live one. This is the display swapping a
 * label, not the data changing its mind.
 */
export const RUNNING_CHIP = {
  state: "live", label: "Researching now",
  long: "The classifier is reading this company right now.",
};
