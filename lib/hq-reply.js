...
export function assistantText({ userText, resultText, outputs }) {
  ...
  // The proposal agent's own words, never the model's retelling of them. A
  // paraphrase loses the options the question box turns into buttons.
  const agent = list.find((item) => item.agent && item.message);
  let text = "";
  if (withFile) text = withFile.message || String(resultText ?? "").trim();
  else if (refused) text = refused.message;
  else if (agent) text = agent.message;
