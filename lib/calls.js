...
/**
 * One dial, however many outcomes it ended in.
 *
 * `logCall` posts one `log_call` per ticked checkbox — "no answer, left a
 * voicemail, sent the follow-up email" is one call and three rows — so a
 * `calls.length` anywhere is a count of outcomes wearing the word "calls".
 * Measured 20 Aug 2026: 16 rows, 11 dials; the NYC list read 13 and was 8.
 *
 * A dial is one person on one day. The three July calls that predate
 * `call_contacts` have no contact id and key on the name they carry, which is
 * the only identity they have — same fallback `reached_people` makes.
 */
export const dialCount = (calls) =>
  new Set(calls.map((c) => `${c.contact_id ?? c.prospect_name}\u0000${c.call_date}`)).size;

/**
 * The call metrics, computed once so tiles and list filters agree.
 *
 * Two definitions that are easy to get wrong later:
 * - "Spoke to someone" = DISTINCT contacts with at least one call whose
 *   outcome is a live contact — not 'no_answer', and not a voicemail or
 *   email left in lieu of one. It is not the number of calls, and it is NOT
 *   the Overview's "People reached", which since 20 Aug counts anyone we
 *   emailed or dialled whatever the outcome. Narrower fact, different name.
 ...
 */
const NOT_REACHED = new Set(["no_answer", "left_voicemail", "left_email"]);
