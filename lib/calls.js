 ...
 *
 * Five outcomes since 21 Aug 2026, and three of them mean you got through:
 * `booked_meeting`, `follow_up` and `not_interested` are all conversations.
 * The other two are the two ways of not getting through — `not_reached` (which
 * includes leaving a voicemail) and `emailed_and_called` (you rang and sent an
 * email, and still spoke to nobody). Both are excluded from "Spoke to someone",
 * which is the only reason they can be split without any tile changing meaning.
 *
 ...
 */
/** The outcomes that are not a conversation. Two of the five, and the one
...
export const NOT_REACHED = new Set(["not_reached", "emailed_and_called"]);
