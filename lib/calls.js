import { db, today, repList } from "./db.js";
...
/**
 * Which rep a call is. Two doors, in this order: the rep who dialled, or the
 * owner of the call list the contact sits on. A call carries no campaign_id —
 * this is the same pair `meeting_rows` and `reached_people` scope by.
 *
 * Exported because the Overview's "Calls logged" tile and the call log behind
 * it both filter by it. Two copies of this line is exactly how a number and
 * the list it opens come to describe different piles.
 */
export const callOwnerOf = (call, ownerOfList) =>
  call.rep ?? ownerOfList.get(call.call_contacts?.call_campaign_id) ?? null;

/** The `call_campaigns.owner` map `callOwnerOf` needs, keyed by campaign id. */
export async function callListOwners() {
  const { data } = await db.from("call_campaigns").select("id, owner, slug, display_name");
  return {
    ownerOf: new Map((data ?? []).map((c) => [c.id, c.owner?.trim() || null])),
    listOf: new Map((data ?? []).map((c) => [c.id, c])),
  };
}

/**
 * Every logged call in a window, newest first, with the person rung and the
 * list they sit on. These are the rows the Overview's "Calls logged" counts —
 * the same table, the same `deleted_at` filter, the same window.
 *
 * Not scoped by rep here: the caller applies `callOwnerOf`, because the rule
 * needs the campaign-owner map and there is one of that too.
 */
export async function callLog({ from, to } = {}) {
  let q = db
    .from("phone_calls")
    .select(
      "id, call_date, prospect_name, company, outcome, note, rep, callback_date, contact_id," +
        " call_contacts(id, full_name, role, org_name, phone, email, call_campaign_id)"
    )
    .is("deleted_at", null)
    .order("call_date", { ascending: false })
    .order("created_at", { ascending: false });
  if (from) q = q.gte("call_date", from).lte("call_date", to);
  const { data } = await q;
  return data ?? [];
}
...
/**
 * The same human's calls, logged on somebody else's list.
 *
 * A person can be on two call lists and 37 of them are: an engineer who signs
 * for a SAFE building and an UNSAFE one is on both, as two `call_contacts`
 * rows, because a list is a list of buildings' people. A call is filed against
 * the one row the rep had open, so opening the other list shows them as never
 * called — which is how the same man gets rung twice in a week.
 *
 * This is a cross-reference, deliberately NOT a call of this campaign's:
 * `callStats` still counts only calls filed here, so the two lists cannot both
 * claim one dial and add up to more than the Overview's total. The panel says
 * where the call actually lives and links to it.
 *
 * Matched on lowercased email AND on name, not one or the other: the same
 * engineer is often enriched on one list and bare on the other — Levon
 * Shaginyan carries an email on the SAFE list and none on the UNSAFE one — so
 * keying on email alone silently misses a third of the overlap.
 */
export async function callsElsewhere(contacts, campaignId) {
  const keys = (c) => [
    (c.email ?? "").trim().toLowerCase(),
    `name:${(c.full_name ?? "").trim().replace(/\s+/g, " ").toLowerCase()}`,
  ].filter((k) => k && k !== "name:");
  const wanted = new Map();
  for (const c of contacts) for (const k of keys(c)) if (!wanted.has(k)) wanted.set(k, c.id);
  if (!wanted.size) return new Map();

  const { data } = await db
    .from("phone_calls")
    .select("id, call_date, outcome, note, rep, call_contacts!inner(id, full_name, email, call_campaign_id)")
    .is("deleted_at", null)
    .neq("call_contacts.call_campaign_id", campaignId)
    .order("call_date", { ascending: false })
    .order("created_at", { ascending: false });

  const out = new Map();
  for (const call of data ?? []) {
    const id = keys(call.call_contacts).map((k) => wanted.get(k)).find(Boolean);
    if (!id) continue;
    if (!out.has(id)) out.set(id, []);
    out.get(id).push(call); // already newest-first
  }
  return out;
}
 ...
 *
 * Six outcomes since 21 Aug 2026, and three of them mean you got through:
 * `booked_meeting`, `follow_up` and `not_interested` are all conversations.
 * The other three are the three ways of not getting through — `not_reached`
 * (nobody picked up, a voicemail counts here), `left_email` and `made_call`.
 * All three are excluded from "Spoke to someone", which is the only reason they
 * can be split apart without any tile changing meaning.
 *
 ...
 */
/** The outcomes that are not a conversation. Three of the six, and the one
 *  place that decides it — `/calls/log` imports this rather than testing the
 *  string itself, so "got through" cannot mean two different things on two
 *  pages. Mirrors `call_outcomes()` in migration 20260821200000. */
export const NOT_REACHED = new Set(["not_reached", "left_email", "made_call"]);
export const gotThrough = (outcome) => !NOT_REACHED.has(outcome);
