 ...
 * unchanged, and in document.xml only that photo's run changes.
 */
export async function replacePhoto(docx, rel, png, { width, height }) {
  ...
  const m = [...doc.matchAll(DRAWING)].find((d) => d[0].match(PHOTO_REL)?.[1] === rel);
  if (!m) throw new Error(`No drawing uses ${rel}`);
  ...
  // The template draws a red box over the photo, placed in points on the page,
  // so it cannot follow the building into a new picture. It sits in the
  // photo's own run, just before the drawing; the edited version drops it.
  const run = Math.max(doc.lastIndexOf("<w:r>", m.index), doc.lastIndexOf("<w:r ", m.index));
  const lead = doc
    .slice(run, m.index)
    .replace(/<mc:AlternateContent>[\s\S]*?<\/mc:AlternateContent>/g, (box) => (box.includes("<v:rect") ? "" : box));

  zip.file(target, png);
  zip.file(DOC, doc.slice(0, run) + lead + drawing + doc.slice(m.index + m[0].length));
