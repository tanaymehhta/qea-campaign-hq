...
export function toolsFor({ email, repName, threadId, userText }) {
  return {
    ...
    hq_campaigns: tool({
      description: "Live campaign groups for the signed-in rep only. No name argument. These counts win over the vault.",
      inputSchema: z.object({}),
      execute: async () => {
        try {
          const campaigns = await campaignsForRep(repName);
          return { campaigns };
        } catch {
    ...
    draft_proposal: tool({
      ...
      execute: async ({ client, address, details }) => {
        ...
        const extra = details && namedByUser(userText, details) ? details : "";
        try {
          return await draftProposal({ threadId, email, client, address, details: extra });
        } catch {
