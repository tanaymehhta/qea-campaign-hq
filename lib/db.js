...
/** The four piles, and the flag on `response_people` that defines each. */
export const PILES = {
  responded:   { flag: "responded",   label: "Total responses" },
  interested:  { flag: "interested",  label: "Interested" },
  needs_label: { flag: "needs_label", label: "Needs a label" },
  all:         { flag: null,          label: "All inbound" },
};

const pileArgs = ({ from, to, campaignIds, source = "instantly" }) => ({
  p_from: from ?? null,
  p_to: to ?? null,
