...
export const METRICS = {
  ...
  contacted:         { label: "People reached", table: "people", rpc: "reached_people",
                       ...
                       note: "First touch per person, both tools — the moment they entered a sequence, not a follow-up. Instantly never exposes first-touch, so a July date can read up to a week late; lemlist's are exact." },
...
/** The tile: {people, instantly, lemlist}. Nulls on error, never zeros. */
export async function reachedCounts(scope) {
  const { data, error } = await db.rpc("reached_counts", pileArgs({ ...scope, source: scope.source ?? null }));
  // Same reasoning as responseCounts: a failed read is not "we emailed nobody".
  if (error || !data?.[0]) return { people: null, instantly: null, lemlist: null };
  return data[0];
}
