...
export const METRICS = {
  ...
  // The only metric whose scope is a function argument rather than a filter on
  // rows. A meeting can arrive through an email campaign, a rep's group or a
  // phone call, and a call meeting has neither of the first two — which is how
  // the tile came to read 5 while this list opened 4. `meeting_rows` answers
  // for all three, and the tile counts `meeting_counts` over the same
  // arguments. Migration 20260820174533.
  meetings:          { label: "Meetings booked", table: "meetings", dateField: "booked_on",
                       note: "Logged by hand — no tool records this. A window means booked in that window, not held in it: a meeting agreed today for September counts today. The four logged before 20 Aug 2026 have no record of when they were agreed and fall back to their meeting date." },
...
export const meetingArgs = ({ from, to, campaignIds, groupIds, rep }) => ({
  p_from: from ?? null,
  p_to: to ?? null,
