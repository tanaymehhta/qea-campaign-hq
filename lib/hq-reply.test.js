...
import { forcedCall, namedByUser, proposalRoute } from "./hq-tools.js";
...
test("a proposal stays with the proposal agent", () => {
  // The bare chip names nobody, so it is a question for the model, not a refusal.
  assert.equal(proposalRoute("Create a proposal", false).to, "model");
  const start = proposalRoute("Create a proposal for Ada at 1 Main St", false);
  assert.equal(start.to, "service");
  assert.equal(start.fresh, true);
  assert.match(start.message, /Ada at 1 Main St/);
  const next = proposalRoute("Yes, the height is 20 m.", true);
  assert.equal(next.to, "service");
  assert.equal(next.fresh, false);
  assert.equal(next.message, "Yes, the height is 20 m.");
  assert.equal(proposalRoute("Research a campaign", true).to, "other");
  assert.equal(proposalRoute("What is the capital of France?", false).to, "model");
});
...
test("the proposal agent's words are not retold by the model", () => {
  const block = "Drone amount? (typical ~$5,300) ($4,500, $5,300, $6,000) [default $5,300]";
  const text = assistantText({
    userText: "Bob Jones, 444 Somerville Ave",
    resultText: "The proposal agent needs a few more details: **Drone amount** — options: ...",
    outputs: [{ agent: true, refused: false, message: block }],
  });
