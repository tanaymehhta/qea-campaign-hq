...
/**
 * Every recent pull request, keyed by the branch it was opened from. One call
 * for the whole page.
 *
 * Listed and matched here rather than asked for with ?head=, because that
 * filter stops matching once the branch is deleted — and deleting the branch is
 * exactly how a change gets turned down. The pull request itself remembers the
 * ref it came from forever, so reading the list and matching on head.ref is the
 * version that keeps working after a no.
 */
async function pullsByBranch() {
  const data = await json("/pulls?state=all&per_page=100&sort=created&direction=desc");
  // null, not {}: "GitHub would not answer" and "there are no pull requests"
  // lead to opposite conclusions, and a card that reports the first as the
  // second tells someone their finished change is still being worked on.
  if (!Array.isArray(data)) return null;
  const out = {};
  for (const pr of data) {
    const ref = pr.head?.ref;
    if (ref?.startsWith("feedback/") && !out[ref]) out[ref] = pr;
  }
  return out;
}
...
export async function runStates(rows) {
  ...
  const [runs, pulls] = await Promise.all([runsByFeedback(), pullsByBranch()]);
  const pairs = await Promise.all(
    asked.map(async (f) => [
      f.id,
      pulls === null
        ? { phase: "unknown", since: f.asked_at }
        : await stateFor(f, runs[f.id], pulls[`feedback/${f.id}`]),
    ]),
  );
  return Object.fromEntries(pairs);
}

async function stateFor(f, run, pr) {
...
/**
 * How many changes are built and waiting on a yes or a no. Called from the
 * layout, so it runs on every page in the dashboard.
 *
 * The database question comes first and is the cheap one: feedback that was
 * dispatched and is not marked done. Almost always nothing, and nothing means
 * no call to GitHub at all — the nav must not make every page in the dashboard
 * wait on a third party to render.
 */
export async function readyToReview(db) {
  const { data } = await db
    .from("feedback")
    .select("id, asked_at")
    .eq("status", "open")
  ...
  const states = await runStates(data);
  return Object.values(states).filter((s) => s.phase === "ready").length;
}
