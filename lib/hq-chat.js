...
export async function getThread(email, id) {
  const { data, error } = await staffDb()
    .from("hq_threads")
    .select("id, email, title, created_at, accepted_at, cost_usd, context_tokens, context_limit")
    .eq("id", id)
    .eq("email", email)
    .maybeSingle();
  if (error) throw new Error(error.message);
...
/**
 * Add one turn's model spend to the thread. Context is replaced, not added:
 * it is how full the window was on the last call, not a running total.
 * ponytail: read-then-write, fine while the composer allows one turn at a time.
 */
export async function addSpend(threadId, { cost = 0, contextTokens = null, contextLimit = null }) {
  const { data, error } = await staffDb()
    .from("hq_threads")
    .select("cost_usd")
    .eq("id", threadId)
    .maybeSingle();
  if (error) throw new Error(error.message);
  const patch = { cost_usd: Number(data?.cost_usd ?? 0) + cost };
  if (contextTokens != null) patch.context_tokens = contextTokens;
  if (contextLimit != null) patch.context_limit = contextLimit;
  const { error: failed } = await staffDb().from("hq_threads").update(patch).eq("id", threadId);
 ...
 *
 * Update-then-insert, not a plain update: a QEA person with no app_users row
 * yet still gets their note saved. A blanket upsert can't be used because
 * display_name is NOT NULL (so the insert needs it) and we must not clobber a
 * real display_name on the update path — so display_name is only set on insert,
 * to the email, matching currentUser's fallback.
 * ponytail: read-then-write race if the same person sends two remembers at once;
 * fine while the composer is one turn at a time — upsert if that ever changes.
 */
export async function rememberNote(email, addition) {
  ...
  if ((data ?? []).length === 0) {
    const { error: failed } = await db
      .from("app_users")
      .insert({ email, display_name: email, person_notes: merged.person_notes });
    if (failed) return { ok: false, error: "The note was not saved." };
  }
  return merged;
}
